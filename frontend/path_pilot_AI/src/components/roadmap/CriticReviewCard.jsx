...
function CriticReviewCard({ review }) {
  return (
    <section className="critic-review-card">
      ...
      <div className="review-group"><h3>Issues found</h3><ul>{review.issues.map((issue) => <li key={issue}><span>!</span>{issue}</li>)}</ul></div>
      <div className="review-group review-group--changes"><h3>Changes made</h3><ul>{review.changesMade.map((change) => <li key={change}><span>✓</span>{change}</li>)}</ul></div>
