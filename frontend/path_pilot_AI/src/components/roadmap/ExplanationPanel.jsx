...
import { formatExplanationPanelTitle } from '../../services/explanationApi.js'

function ExplanationPanel({ error, explanation, item, loading, onClose, onRetry }) {
  const displayTitle = formatExplanationPanelTitle(item)
  return (
    <div className="explanation-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <aside className="explanation-panel" aria-labelledby="explanation-title" aria-modal="true" role="dialog">
        <div className="explanation-heading"><div><p className="roadmap-kicker">WHY THIS MATTERS</p><h2 id="explanation-title">{displayTitle}</h2></div><button aria-label="Close explanation" onClick={onClose} type="button"><FontAwesomeIcon aria-hidden="true" icon={faXmark} /></button></div>
