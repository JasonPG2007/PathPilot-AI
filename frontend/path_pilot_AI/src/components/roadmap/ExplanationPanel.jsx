  ...
  return (
    <div className="explanation-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <aside className="explanation-panel" aria-labelledby="explanation-title" aria-modal="true" role="dialog">
        <div className="explanation-heading"><div><p className="roadmap-kicker">WHY THIS MATTERS</p><h2 id="explanation-title">{item}</h2></div><button aria-label="Close explanation" onClick={onClose} type="button">×</button></div>
        {loading && <div className="explanation-loading" role="status"><span aria-hidden="true" />Explaining this roadmap decision…</div>}
        {error && <div className="explanation-error" role="alert"><p>{error}</p><button className="button" onClick={onRetry} type="button">Retry</button></div>}
