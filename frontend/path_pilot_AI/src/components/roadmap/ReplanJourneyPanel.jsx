  ...
  return (
    <div className="replan-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <aside className="replan-panel" aria-labelledby="replan-title" aria-modal="true" role="dialog">
        <div className="replan-panel-heading"><div><p className="roadmap-kicker">ADAPTIVE REPLANNING</p><h2 id="replan-title">Replan My Journey</h2></div><button aria-label="Close replan panel" onClick={onClose} type="button"><FontAwesomeIcon aria-hidden="true" icon={faXmark} /></button></div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="replan-panel-actions"><button className="button button--secondary" disabled={submitting} onClick={onClose} type="button">Cancel</button><button className="button" disabled={submitting} type="submit"><FontAwesomeIcon aria-hidden="true" className={submitting ? 'status-icon--spinning' : ''} icon={faArrowsRotate} /> {submitting ? 'Replanning…' : 'Create Revised Plan'}</button></div>
        </form>
      </aside>
    </div>
  )
