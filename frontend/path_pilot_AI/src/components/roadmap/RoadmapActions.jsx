...
function RoadmapActions({ onOpenReplan, onResetProgress }) {
  ...
  return (
    <section className="roadmap-actions">
      ...
      <div><button className="button" onClick={onOpenReplan} type="button">✦ Replan My Journey</button><button className="button button--secondary" onClick={() => setMessage('Roadmap downloads are coming soon. Your plan is safe on this page.')} type="button">⇩ Download Roadmap</button><Link className="button button--secondary" to="/create">↻ Start Again</Link><button className="text-action reset-progress-action" onClick={onResetProgress} type="button">Reset Progress</button><Link className="text-action" to="/">Back Home</Link></div>
